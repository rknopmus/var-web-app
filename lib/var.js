...
const toNumber = v => {
  ...
  for (let i = 1; i < precios.length; i++) {
    ...
    activos.forEach(a => {
const columnasPrecios = Object.keys(precios[0] || {});

const activoEnPrecios = columnasPrecios.find(
  c => String(c).trim().toUpperCase() === String(a).trim().toUpperCase()
);
...
const precioAnterior = toNumber(precios[i - 1][activoEnPrecios]);
const precioActualFila = toNumber(precios[i][activoEnPrecios]);
const priceNow = toNumber(precios[precios.length - 1][activoEnPrecios]);
const posicion = toNumber(posiciones[a]);
...
const contribucionVaR = {};

activos.forEach(a => {
  const varSinActivo = resultados_total.map((total, i) => {
    const resultadoActivo = resultadosPorActivo[a][i] || 0;
    return total - resultadoActivo;
  });

  const varPortfolioSinActivo =
    method === "historical"
      ? quantile(varSinActivo, 1 - alpha)
      : mean(varSinActivo) + inverseNormal(1 - alpha) * std(varSinActivo);

  const contribucionAbs = TotalVaR - varPortfolioSinActivo;

  contribucionVaR[a] = {
    VaRIndividual: dict_var[a],
    VaRSinActivo: varPortfolioSinActivo,
    ContribucionAbs: contribucionAbs,
    ContribucionPct:
      TotalVaR !== 0 ? (contribucionAbs / TotalVaR) * 100 : null
  };
});
