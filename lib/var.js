...
const sumaVaRAbs = Object.values(dict_var)
  .map(v => Math.abs(v || 0))
  .reduce((a, b) => a + b, 0);
...
activos.forEach(a => {
  const varAbs = Math.abs(dict_var[a] || 0);

  contribucionVaR[a] = {
    VaR: dict_var[a],
    ContribucionAbs: varAbs,
    ContribucionPct: sumaVaRAbs !== 0 ? (varAbs / sumaVaRAbs) * 100 : null
  };
});
