export function calcularVaR(precios, posiciones, alpha = 0.95, method = "historical") {
  ...
  function inverseNormal(p) {
    // Aproximación de Peter John Acklam
    const a = [
      -39.69683028665376,
      220.9460984245205,
      -275.9285104469687,
...
const TotalESFMinus =
  colaNegativa.length > 0
    ? mean(colaNegativa)
    : null;

const TotalESFPlus =
  colaPositiva.length > 0
    ? mean(colaPositiva)
    : null;

const RatioEaRVaRAbs =
  TotalVaR !== 0
    ? Math.abs(TotalEaR / TotalVaR)
    : null;
...
return {
  ...
  if (method === "parametric_normal") {
    ...
    activos.forEach(a => {
      ...
      dict_var[a] = mu + zLeft * sigma;
    });

    const muTotal = mean(resultados_total);
    const sigmaTotal = std(resultados_total);

    const TotalVaR = muTotal + zLeft * sigmaTotal;
    const TotalEaR = muTotal + zRight * sigmaTotal;
